...
Further examples of gridSVG have been created as part of my [Masters Thesis](/projects/msc-thesis/). In particular these can be run via the `sjpMScThesis` package that is a companion package to my thesis. The key difference that these examples offer is that the content of the gridSVG image is being updated via an R web server, enabling reactive statistical graphics. Videos of these examples are linked below:

* [Hexagonal binning](/projects/msc-thesis/videos/hexbin.webm) --- A comparison of encoding the density of data in a plot by area or colour.
* [ARIMA Diagnostics](/projects/msc-thesis/videos/arima.webm) --- Interactive ARIMA model diagnostics. ACF and PACF plots dynamically update in response to changes in any of the p, d or q parameters in an ARIMA model.
* [LOESS Smoothing](/projects/msc-thesis/videos/loess.webm) --- A simple example which easily enables a user to determine a suitable value for controlling the degree of smoothing applied.
* [Multidimensional Scaling](/projects/msc-thesis/videos/mds.webm) --- An interactive example showing the effect of multidimensional scaling on the true pairwise distances between european cities.
* [Sampling Variation Teaching Example](/projects/msc-thesis/videos/sampling-variation.webm) --- A reimplementation of the sampling variation animation that is created by the [Visual Inference Tools](/projects/visual-inference-tools/) package. Key improvements are that animation timing is accurate and the animations themselves are smooth.
